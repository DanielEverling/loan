...
describe('customer repository sequelize', () => {
  ...
  describe('update address method', () => {
    it('should be able to replace a customer address on the database', async () => {
      ...
      const savedAddress = await customerRepository.updateAddress(
        newAddress,
        customer.id,
      );
  ...
  describe('find method', () => {
    it('should be able to find by ssn', async () => {
      const ssNumber = SSNumber.of('111111111');

      const customer = makeCustomerEntity({ ssNumber });

      await customerRepository.save(customer);

      const foundCustomer = await customerRepository.findBySSNumber(ssNumber);

      expect(foundCustomer.ssNumber).toEqual(ssNumber);
    });
  });
});
